...
PWA는 앞의 세 층과 성격이 다르다. 1~3층이 "어떻게 느껴지는가"를 만들었다면, 이 층은 그렇게 만든 UX를 설치 가능하고 오프라인에서도 도는 앱으로 *포장*한다. 그래서 새로운 디자인이 아니라 능력의 추가이고, 네 층 중 프레임워크가 가장 덜 중요한 층이다 — 실체가 웹 플랫폼 표준(서비스 워커·manifest·캐시)이라 누가 만들든 같은 API 위에서 돈다.

한 가지 오해를 먼저 풀어 두면 좋다. PWA는 "모바일용 웹사이트"가 아니다. 평범한 웹앱에 설치와 오프라인이라는 앱 능력을 점진적으로 덧입히는 것이고, 데스크톱에도 똑같이 설치된다. 모바일과 자주 엮이는 것은 정의가 아니라 쓰임새의 강조일 뿐이다.
...
오프라인 캐싱을 생각하다 보면 자연스레 SSR로 눈이 가지만, 둘은 다른 관심사다. PWA는 이미 만든 CSR 앱 위에 덧입히는 능력인 반면, SSR은 렌더링 모델 자체를 바꾸는 침습적 작업이라 클라이언트 전용 가정(`localStorage`·`IndexedDB`·`window`·제스처)을 전부 건드린다. 그래서 SSR은 이 스킬의 범위 밖이며, 완성된 이 CSR 앱을 기준점으로 삼는 별도 작업으로 다루는 편이 깨끗하다. 다만 지금부터 클라이언트 전용 동작의 렌더 시점을 명확히 두면(`afterNextRender` 등), 그때 옮길 여지가 남는다.
